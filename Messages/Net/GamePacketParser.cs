...
namespace Polar.Net
{
    public class GamePacketParser : IDataParser, IDisposable, ICloneable
    {
        ...
        public GamePacketParser(GameClient me)
        {
            ...
            // Aumentar el tamaño inicial del buffer
            this._webSocketBuffer = new byte[1228800]; // Ajusta este valor según sea necesario
        }
        ...
        public void handlePacketData(byte[] Data, bool deciphered = false)
        {
            try
            {
                ...
                if (Data.Length == 23 || Data.Length == 22)
                {
                    ...
                    {
                        var responseBytes = Encoding.UTF8.GetBytes(GetXmlPolicy());
        ...
        private byte[] DecodeSingleWebSocketFrame(byte[] data, int startPosition, out int frameLength)
        {
            ...
            try
            {
                ...
                else if (dataLength == 127)
                {
                    ...
                    dataLength = (data[startPosition + 6] << 24) | (data[startPosition + 7] << 16) | (data[startPosition + 8] << 8) | data[startPosition + 9];
                    indexFirstMask = 10;
                }
        ...
        {
            // Verificar que OnNewPacket no sea null antes de procesar
            if (this.OnNewPacket == null)
                return;
            ...
            Data[position], Data[position + 1], Data[position + 2], Data[position + 3]
                ...
                if (Packet.Length >= 2)
                {
                    ...
                    // Verificar una última vez antes de invocar
                    ...
                    {
                        // Si se perdió el handler, salir del bucle
                        break;
                    }
                }
        ...
        private void PolicyRequest(byte[] packet)
        {
            ...
            if (!data.Contains("ey:"))
                return;
            string key = data.Replace("ey:", "`").Split('`')[1].Replace("\r", "").Split('\n')[0].Trim();
